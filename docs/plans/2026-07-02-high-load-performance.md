# High-Load Performance & FPS Benchmarking Implementation Plan

> **For Antigravity:** REQUIRED SUB-SKILL: Load executing-plans to implement this plan task-by-task.

**Goal:** Build a benchmark test suite that runs a simulation with 100+ active links and entities (nuclear hazard areas, projectiles, hubs) to measure engine sub-tick tick rates and Canvas frame rendering performance.

**Architecture:** Populates a `GameState` with 100+ entities (hubs, links, nuclear hazards, projectiles), runs physics tick updates, intercepts JSDOM's canvas creator to return a stubbed/mocked high-performance Canvas 2D context, and benchmarks both the sub-tick resolution time and canvas rendering time, printing results in a formatted console table.

**Tech Stack:** Vitest, JSDOM, GameState simulation engine, Canvas 2D Renderers.

---

### Task 1: High-Load Performance Benchmark Suite

**Files:**
- Create: `shared/tests/HighLoadPerformance.test.js`

**Step 1: Write the failing test**
Create a new file `shared/tests/HighLoadPerformance.test.js` with a failing assertion that checks for the benchmark's run status.
...
describe('High-Load Performance Benchmark', () => {
    it('runs the high-load simulation and measures performance', () => {
        expect(false).toBe(true); // Initial failing test
    });
});
```

**Step 2: Run test to verify it fails**
Run: `npx vitest run shared/tests/HighLoadPerformance.test.js`
Expected: FAIL with "expected false to be true"

**Step 3: Write minimal implementation**
Implement the full benchmark logic inside `shared/tests/HighLoadPerformance.test.js`.
...
describe('High-Load Performance Benchmark', () => {
    it('runs the high-load simulation and measures performance', () => {
        ...
        game.map.lakes = Array.from({ length: 5 }, (_, i) => ({ x: 200 + i * 300, y: 200 + i * 250, radius: 100 }));
        game.map.mountains = Array.from({ length: 5 }, (_, i) => ({ x: 400 + i * 300, y: 300 + i * 200, radius: 80 }));
        game.map.resources = Array.from({ length: 15 }, (_, i) => ({ x: 100 + i * 120, y: 150 + i * 110, radius: 8, value: 5 }));

        // 2. Add 20 Hubs (10 owned by p1, 10 owned by p2)
        const hubs = [];
        for (let i = 0; i < 10; i++) {
        ...
        // 3. Add 40 active links
        for (let i = 0; i < 20; i++) {
            game.addLink(hubs[i * 2].id, hubs[(i * 2 + 1) % hubs.length].id, 'p1');
            game.addLink(hubs[i * 2 + 1].id, hubs[(i * 2 + 2) % hubs.length].id, 'p2');
        }

        // 4. Add 10 Nuclear Hazard Areas
        for (let i = 0; i < 10; i++) {
            game.addEntity({
        ...
        // 5. Add 30 projectiles flying in toroidal space
        for (let i = 0; i < 30; i++) {
        ...
        const durationResolution = endResolution - startResolution;
        ...
        console.log(`- Turn Resolution:    ${durationResolution.toFixed(2)} ms`);
        ...
        expect(durationResolution).toBeLessThan(5000); // Sanity check
        expect(avgRenderTime).toBeLessThan(100);       // Sanity check
    });
});
```
...
Run: `npx vitest run shared/tests/HighLoadPerformance.test.js`
Expected: PASS and prints the ASCII performance metrics table.

**Step 5: Commit**
```bash
git add -f shared/tests/HighLoadPerformance.test.js docs/plans/2026-07-02-high-load-performance.md
git commit -m "test: add high-load performance & FPS benchmarking suite"
```
