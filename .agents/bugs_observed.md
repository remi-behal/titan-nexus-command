...
1. You can launch napalm at such a short range that you can land it on the launch structure. We need a minimum range for napalm. Fixed [x]
2. Napalm projectile isn't being removed, you can see in terminal log that it is being destroyed by the napalm fire. Fixed [x]
3. When a turn only has 1 round, the napalm fire is only doing 1 damage for that round. It needs to somehow initiate another round and another 1 damage in it. Fixed [x]
