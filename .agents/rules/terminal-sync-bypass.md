...
- _Note: Always read and write to `[VAR_TEMP_DIR]`. This avoids out-of-workspace permission prompts while keeping temporary execution files organized._
...
- _Why? `tee` (without `-a`) overwrites the file for each new command within the same conversation, keeping the file small. It ensures the output is visible to the user in the IDE terminal while also writing it securely to the file for the agent to verify._
...
- _Ignore Status Hangs_: Never wait for `command_status` to report `DONE` if `view_file` confirms the command has finished executing. If the command appears stuck in the UI, disregard it and move to the next step.
