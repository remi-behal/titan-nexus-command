...
- **Direction**: Pull back _away_ from the target. The further the pull, the higher the power.
...
- **Mountains**: Obstacles that cannot be built on, though links _can_ cross them.
...
- **Round Execution Loop**:
    ...
    2.  **Integrity Check**: If a source structure was destroyed in a previous round, the action is discarded, and the server pulls the _next_ item in that player's queue for the same round.
