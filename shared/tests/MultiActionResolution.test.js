...
describe('GameState - Multi-Action Resolution', () => {
    ...
    it('should launch actions from unique hubs in the same round', () => {
        ...
        const round1Sub = snapshots.find(
            (s) => s.type === 'ROUND_SUB' && s.round === 1 && s.subTick === 6
        );

        // We expect exactly 2 projectiles in Round 1 (one from Hub A, one from Hub B)
        const round1ProjCount = round1Sub.state.entities.filter(
            (e) => e.type === 'PROJECTILE'
        ).length;
        ...
        const round2Sub = snapshots.find(
            (s) => s.type === 'ROUND_SUB' && s.round === 2 && s.subTick === 6
        );
        expect(round2Sub).toBeDefined();
        const round2ProjCount = round2Sub.state.entities.filter(
            (e) => e.type === 'PROJECTILE'
        ).length;
    ...
    it('should only fire defense once per round even with multiple fuel (overwhelming)', () => {
        ...
        const subSnap = snapshots.find(
            (s) => s.type === 'ROUND_SUB' && s.round === 1 && s.subTick === 12
        );
