...
describe('GameState - Cluster Bomb', () => {
    ...
    it('should sub-bombs land in a line perpendicular to travel', () => {
            ...
            if (s.type === 'ROUND_SUB') {
                s.state.entities.forEach((e) => {
                    if (e.type === 'EXPLOSION') {
                        ...
                        if (
                            !explosions.some(
                                (ex) => Math.abs(ex.x - e.x) < 1 && Math.abs(ex.y - e.y) < 1
                            )
                        ) {
        ...
        explosions.forEach((ex) => {
            ...
            expect(ex.y).toBeGreaterThanOrEqual(
                500 - ENTITY_STATS.CLUSTER_BOMB.spreadDistance / 2 - 1
            );
            expect(ex.y).toBeLessThanOrEqual(
                500 + ENTITY_STATS.CLUSTER_BOMB.spreadDistance / 2 + 1
            );
        });
