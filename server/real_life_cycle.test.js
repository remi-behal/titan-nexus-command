...
describe('Full Cycle Integration - Real Life Scenarios', () => {
    it('should complete a 5-turn complex scenario', async () => {
        ...
        p1.on('playerAssignment', (id) => {
            console.log('p1 assigned:', id);
            p1Id = id;
        });
        p2.on('playerAssignment', (id) => {
            console.log('p2 assigned:', id);
            p2Id = id;
        });
        ...
        try {
            ...
            console.log(
                `Turn 2 Start: p1(${p1Id}) has ${p1HubsTurn2} hubs, p1 energy: ${p1State.players[p1Id].energy}`
            );
            ...
            await waitFor(() => {
                ...
                if (Date.now() % 2000 < 200)
                    console.log(`Turn 2 Sync: p1 has ${hubs} hubs, phase: ${phase}`);
            ...
            console.log(
                `p1(${p1Id}) striking p2(${p2Id}) hub ${hub2p2.id} at (${Math.round(targetX)}, ${Math.round(targetY)})`
            );
            console.log(
                `Relative Vector: dx=${Math.round(dx)}, dy=${Math.round(dy)}, Angle=${angleToP2.toFixed(1)}, Dist=${distToP2.toFixed(1)}`
            );
            ...
            console.log(
                `Calculated Pull Distance for ${Math.round(distToP2)}px launch: ${pullDistance.toFixed(1)}`
            );
            ...
            await waitFor(() => {
                ...
                if (Date.now() % 2000 < 200) {
                    ...
                    console.log(
                        `Waiting for Turn 3... Current Turn: ${turn}, P2 Hub HP: ${h2?.hp}`
                    );
                }
            ...
            console.log(
                `Hub 2 status: ${!!h2p1Latest}, Deployed: ${h2p1Latest?.deployed}, HP: ${h2p1Latest?.hp}, Energy: ${p1State.players[p1Id].energy}, Phase: ${p1State.phase}`
            );
            ...
            if (p1Extractors.length === 0) {
                console.log(
                    `P1 Entities at Turn 4: ${p1State.entities.map((e) => `${e.type}:${e.owner}:${e.id}`).join(', ')}`
                );
            }
