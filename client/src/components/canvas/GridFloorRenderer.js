...
export function drawGridFloor(ctx, map, viewBounds, offsetOffsetX, offsetOffsetY) {
    ...
    if (map.lakes) {
        map.lakes.forEach((lake) => {
            if (
                lake.x + offsetOffsetX + lake.radius < viewL ||
                lake.x + offsetOffsetX - lake.radius > viewR ||
                lake.y + offsetOffsetY + lake.radius < viewT ||
                lake.y + offsetOffsetY - lake.radius > viewB
            )
                return;
    ...
    if (map.mountains) {
        map.mountains.forEach((mtn) => {
            if (
                mtn.x + offsetOffsetX + mtn.radius < viewL ||
                mtn.x + offsetOffsetX - mtn.radius > viewR ||
                mtn.y + offsetOffsetY + mtn.radius < viewT ||
                mtn.y + offsetOffsetY - mtn.radius > viewB
            )
                return;
    ...
    if (map.craters) {
        map.craters.forEach((crater) => {
            if (
                crater.x + offsetOffsetX + crater.radius < viewL ||
                crater.x + offsetOffsetX - crater.radius > viewR ||
                crater.y + offsetOffsetY + crater.radius < viewT ||
                crater.y + offsetOffsetY - crater.radius > viewB
            )
                return;
    ...
    if (map.resources) {
        map.resources.forEach((res) => {
            if (
                res.x + offsetOffsetX + (res.radius || 8) < viewL ||
                res.x + offsetOffsetX - (res.radius || 8) > viewR ||
                res.y + offsetOffsetY + (res.radius || 8) < viewT ||
                res.y + offsetOffsetY - (res.radius || 8) > viewB
            )
                return;
            ...
            drawShape(
                ...
                res.x,
                res.y,
                isSuper ? 'SUPER_RESOURCE_NODE' : 'RESOURCE_NODE',
                res.radius || 8,
                isSuper ? '#a020f0' : '#ffa500',
                0,
                false
            );
        });
